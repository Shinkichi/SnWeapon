...
simulated state WeaponBurstFiring
{
	...
	simulated function PlayFireEffects( byte FireModeNum, optional vector HitLocation )
	{
		...
		if( FireModeNum != DEFAULT_FIREMODE || (FireModeNum == DEFAULT_FIREMODE &&
		...
		if( Instigator != none )
		{
			...
			{
				if ( !bPlayingLoopingFireAnim && (FireModeNum != DEFAULT_FIREMODE || (FireModeNum == DEFAULT_FIREMODE &&
...
{
	local KFWeap_AssaultRifle_TacticalRifle Weap;

	Super.SetOriginalValuesFromPickup(PickedUpWeapon);

	if(Role == ROLE_Authority && !Instigator.IsLocallyControlled())
	{
		Weap = KFWeap_AssaultRifle_TacticalRifle(PickedUpWeapon);
...
defaultproperties
{
	...
	SpareAmmoCapacity[0]= 210//270
    //grenades
	InitialSpareMags[1]	= 1//3
	MagazineCapacity[1]	= 1
	SpareAmmoCapacity[1]= 8//13 //11
	...
	BurstAmount=3
	BurstFire2RdAnim=Shoot//Shoot_Burst2
	BurstFire3RdAnim=Shoot//Shoot_Burst
	BurstFire2RdSightedAnim=Shoot_Iron//Shoot_Burst2_Iron
	BurstFire3RdSightedAnim=Shoot_Iron//Shoot_Burst_Iron
	...
	// Fire Effects
	WeaponFireSnd(DEFAULT_FIREMODE)=(DefaultCue=AkEvent'WW_WEP_SA_AK12.Play_WEP_SA_AK12_Fire_Single_M', FirstPersonCue=AkEvent'WW_WEP_SA_AK12.Play_WEP_SA_AK12_Fire_Single_S')
	...
	WeaponFireSnd(2)=(DefaultCue=AkEvent'WW_WEP_SA_AK12.Play_WEP_SA_AK12_Fire_Single_M', FirstPersonCue=AkEvent'WW_WEP_SA_AK12.Play_WEP_SA_AK12_Fire_Single_S')
